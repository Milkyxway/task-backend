...
class ChartsService extends Service {
  getPieCharts(query) {
    const sqlStr = `select count(${query.type}) as value , ${query.type} as name from task_list group by ${query.type}`;
  ...
  getSectionTaskSort() {
    const sqlStr = 'select leadOrg as name, count(leadOrg) as value from task_list group by leadOrg order by count(leadOrg) desc';
    return new Promise(async (resolve, reject) => {
      try {
        const list = await this.app.mysql.query(sqlStr);
        resolve(list);
